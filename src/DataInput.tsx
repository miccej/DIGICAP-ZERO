      ...
      <div className={`${headerClass} px-4 h-12 md:h-14 flex items-center justify-between shrink-0`}>
        ...
        <div className="flex gap-2 items-center">
          ...
          <button 
            type="button" 
            onClick={() => {
              if (isDemoMode && onRestrictedAction) {
                onRestrictedAction();
              } else {
                setShowPasteModal(true);
              }
            }} 
            className="p-1.5 md:p-2 rounded-sm text-slate-400 hover:text-slate-900" 
          ...
          <button 
            ...
            className={`p-1.5 md:p-2 rounded-sm ${showSim ? (themeMode === 'dark' ? 'bg-slate-800 text-slate-100' : 'bg-slate-100 text-slate-900') : 'text-slate-400 hover:text-slate-900'}`} 
            title={t.simulateHeader}
          >
        ...
        <div className={`flex-1 overflow-y-auto mb-4 md:mb-6 border-2 ${activeColor.border} ${themeMode === 'dark' ? 'bg-slate-900' : 'bg-white'} p-2 md:p-3 custom-scrollbar`}>
            ...
            <div className="space-y-4">
                {calculationMethod === 'within' ? (
                          ...
                          {group.map((item, i) => {
                            ...
                            return (
                              <div key={item.id} className={`p-1 text-center font-mono text-[10px] font-bold border ${isOutOfSpec ? 'border-red-500 text-red-500 bg-red-500/5' : 'border-slate-300 text-slate-600'}`}>
                                {item.value.toLocaleString('sv-SE', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}
                              </div>
                            );
                          })}
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-4 gap-2 md:gap-3">
