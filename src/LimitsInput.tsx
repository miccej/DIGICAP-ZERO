...
const LimitsInput: React.FC<LimitsInputProps> = ({ 
  ...
  const formatInputVal = (val: number | undefined): string => {
    if (val === undefined || isNaN(val)) return '';
    return val.toLocaleString('sv-SE', { minimumFractionDigits: 2, maximumFractionDigits: 4 });
  };

  const [localLsl, setLocalLsl] = useState<string>(formatInputVal(limits.lsl));
  const [localUsl, setLocalUsl] = useState<string>(formatInputVal(limits.usl));
  const [localTarget, setLocalTarget] = useState<string>(formatInputVal(limits.target));
  ...
  useEffect(() => {
    setLocalLsl(formatInputVal(limits.lsl));
    setLocalUsl(formatInputVal(limits.usl));
    setLocalTarget(formatInputVal(limits.target));
  }, [measureId]);
  ...
  const handleBlur = (field: 'lsl' | 'usl' | 'target', setLocal: React.Dispatch<React.SetStateAction<string>>) => {
    const val = limits[field];
    if (val !== undefined) {
      setLocal(formatInputVal(val));
    }
  };
  ...
  return (
    <div className={`${containerClass} h-full overflow-hidden`}>
      ...
      <div className="p-3 md:p-4 space-y-3 md:space-y-6 flex-1">
        ...
        <div className="grid grid-cols-[1.5fr_0.5fr] md:grid-cols-[1.4fr_0.6fr] gap-3 md:gap-4">
            <div className="relative group">
                <div className="flex items-center mb-1 h-5 md:h-6">
                    <label className={`flex items-center gap-2 text-[10px] md:text-[12px] font-bold uppercase tracking-wider ${labelColor} select-none`}>
                        {t.calcMethod}
        ...
        <div className="relative group">
            ...
            <div className="relative group">
                ...
                onBlur={() => handleBlur('target', setLocalTarget)}
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4">
            <div className={`transition-all duration-300 relative group ${limits.toleranceType !== 'upper' ? 'opacity-100' : 'opacity-30 pointer-events-none grayscale'}`}>
                ...
                <input
                    ...
                    onBlur={() => handleBlur('lsl', setLocalLsl)}
            ...
            <div className={`transition-all duration-300 relative group ${limits.toleranceType !== 'lower' ? 'opacity-100' : 'opacity-30 pointer-events-none grayscale'}`}>
                ...
                <input
                    ...
                    onBlur={() => handleBlur('usl', setLocalUsl)}
                    className={`w-full px-3 md:px-4 py-1.5 md:py-2 h-[36px] md:h-[40px] text-sm font-mono font-bold transition-all border-2 ${inputClass}`}
                />
            </div>
        </div>
